...
// ve sayısı vardır; renk yalnız gruplandırma içindir. Pasta/donut YOKTUR —
// dilim açısı okunamaz, aynı veri yatay çubukta her zaman daha net.
//
// Tek kopya (ADR 0011).
...
export function barChart(rows, { max = 12 } = {}) {
  ...
  const svg = el('svg', { viewBox: `0 0 ${width} ${height}`, class: 'kit-chart',
    role: 'img' });
...
// ─────────────────────────────────────────────── kit eklemeleri (ADR 0011)

/**
 * Sparkline — satır içi minik eğilim çizgisi.
 *
 * Ekseni yoktur ve OLMAMALIDIR: tek başına okunmaz, yanındaki sayının
 * eğilimini gösterir. Müşteri kartında "son 6 ay harcama", KPI kutusunda
 * "son 30 gün" için.
 *
 * @param {number[]} values
 */
export function sparkline(values, { width = 120, height = 28, tone = ACCENT } = {}) {
  const svg = el('svg', {
    viewBox: `0 0 ${width} ${height}`, class: 'kit-spark',
  ...
  const floor = Math.min(...list, 0);
  const span = peak - floor || 1;
  const stepX = width / (list.length - 1);
  const y = (value) => height - 2 - ((value - floor) / span) * (height - 4);

  const points = list.map((value, index) => `${index * stepX},${y(value)}`).join(' ');
  svg.append(el('polyline', {
    points, fill: 'none', stroke: tone, 'stroke-width': 1.6,
    'stroke-linejoin': 'round', 'stroke-linecap': 'round',
  }));
  // Son nokta vurgulanır: "şu an neredeyiz" sorusunun cevabı.
  svg.append(el('circle', {
    cx: (list.length - 1) * stepX, cy: y(list.at(-1)), r: 2.2, fill: tone,
  }));
  svg.append(el('title', {}, `${list.at(-1)}`));
  return svg;
}
 ...
 *
 * Sipariş durumu dağılımı, yorum puan dağılımı gibi "toplamı 100 eden"
 * veriler için. Her parçanın SAYISI da yazılır; renk tek başına anlam
 * taşımaz kuralı burada da geçerli.
 *
 * @param {Array<{label:string, value:number}>} parts
 */
export function stackedBar(parts, { height = 34, width = 720 } = {}) {
  const svg = el('svg', {
    viewBox: `0 0 ${width} ${height + 26}`, class: 'kit-chart', role: 'img',
  });
  const list = (parts || []).filter((part) => Number(part.value) > 0);
  const total = list.reduce((sum, part) => sum + Number(part.value), 0);

  if (total === 0) {
    svg.append(el('text', {
      x: width / 2, y: height / 2, 'text-anchor': 'middle', fill: SOFT, 'font-size': 12,
    }, 'Veri yok'));
    return svg;
  }
  ...
  list.forEach((part, index) => {
    ...
    // Tonlar aynı vurgudan türer: ayrım için yeterli, gökkuşağı değil.
    const tone = mix(ACCENT, 1 - (index / Math.max(1, list.length)) * 0.62);
    svg.append(el('rect', {
      x: cursor, y: 0, width: Math.max(1, span), height,
      fill: tone, rx: index === 0 || index === list.length - 1 ? 4 : 0,
    })).append(el('title', {}, `${part.label}: ${value}`));

    // Etiket ancak sığıyorsa çubuğun İÇİNE yazılır; sığmazsa altta listelenir.
    if (span > 46) {
      svg.append(el('text', {
        x: cursor + span / 2, y: height / 2 + 4, 'text-anchor': 'middle',
  ...
  // Alt açıklama şeridi: her parçanın adı ve sayısı.
  let legendX = 0;
  list.forEach((part, index) => {
    const tone = mix(ACCENT, 1 - (index / Math.max(1, list.length)) * 0.62);
    svg.append(el('rect', { x: legendX, y: height + 10, width: 9, height: 9, rx: 2, fill: tone }));
    const text = `${part.label} ${part.value}`;
    svg.append(el('text', {
      x: legendX + 13, y: height + 18, fill: SOFT, 'font-size': 10.5,
    }, text));
    legendX += 22 + text.length * 5.6;
  });
  return svg;
}

/**
 * Gruplu dikey çubuk — aynı eksende iki seri karşılaştırması.
 *
 * "Taşıyıcı bazlı ortalama teslim süresi", "bu dönem vs geçen dönem".
 *
 * @param {Array<{label:string, values:number[]}>} groups
 * @param {string[]} series — seri adları (açıklama şeridi)
 */
export function groupedBar(groups, { series = [], height = 210, format = String } = {}) {
  const width = 720;
  const padLeft = 46;
  const padBottom = 34;
  const svg = el('svg', { viewBox: `0 0 ${width} ${height}`, class: 'kit-chart', role: 'img' });
  ...
  const peak = Math.max(1, ...list.flatMap((group) => group.values.map((value) => Number(value) || 0)));
  const plotHeight = height - padBottom - 16;
  const y = (value) => 16 + plotHeight - (value / peak) * plotHeight;

  // Yatay kılavuz çizgileri — göz değeri okusun diye.
  for (let step = 0; step <= 4; step += 1) {
    const value = (peak / 4) * step;
    svg.append(el('line', {
      x1: padLeft, x2: width - 8, y1: y(value), y2: y(value), stroke: LINE, 'stroke-width': 1,
    }));
    svg.append(el('text', {
      x: padLeft - 6, y: y(value) + 3.5, 'text-anchor': 'end', fill: SOFT, 'font-size': 9.5,
    }, format(value)));
  }

  const slot = (width - padLeft - 8) / list.length;
  const seriesCount = Math.max(1, list[0].values.length);
  const barWidth = Math.max(3, (slot * 0.66) / seriesCount);

  list.forEach((group, index) => {
    const base = padLeft + index * slot + slot * 0.17;
    group.values.forEach((raw, seriesIndex) => {
      const value = Number(raw) || 0;
      const tone = mix(ACCENT, 1 - (seriesIndex / Math.max(1, seriesCount)) * 0.55);
      const top = y(value);
      const bar = el('rect', {
        x: base + seriesIndex * barWidth, y: top,
        width: barWidth - 1.5, height: Math.max(1, 16 + plotHeight - top), fill: tone, rx: 2,
      });
      bar.append(el('title', {}, `${group.label} · ${series[seriesIndex] || ''} ${format(value)}`));
      svg.append(bar);
    });
    svg.append(el('text', {
      x: base + (barWidth * seriesCount) / 2, y: height - padBottom + 24,
      'text-anchor': 'middle', fill: SOFT, 'font-size': 10,
    }, group.label));
  });

  let legendX = padLeft;
  series.forEach((name, index) => {
    const tone = mix(ACCENT, 1 - (index / Math.max(1, seriesCount)) * 0.55);
    svg.append(el('rect', { x: legendX, y: 2, width: 9, height: 9, rx: 2, fill: tone }));
    svg.append(el('text', { x: legendX + 13, y: 10, fill: SOFT, 'font-size': 10.5 }, name));
    legendX += 22 + name.length * 5.6;
  });
