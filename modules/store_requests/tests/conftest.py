...
modülün `backend` klasörü `store_requests_backend` adıyla kaydedilir;
içerideki göreli importlar (`.rma`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/store_requests/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_requests_backend" not in sys.modules:
    package = types.ModuleType("store_requests_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_requests_backend"] = package
