...
`backend` klasörü `bbd_canteen_devices_backend` adıyla kaydedilir; içerideki
göreli importlar (`.kiosks`) paketin `__path__` değeri üzerinden çözülür.
"""
...
# çalıştırıldığında (`pytest modules/bbd_canteen_devices`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "bbd_canteen_devices_backend" not in sys.modules:
    package = types.ModuleType("bbd_canteen_devices_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["bbd_canteen_devices_backend"] = package
