...
modülün `backend` klasörü `store_reports_backend` adıyla kaydedilir;
...
# başına çalıştırıldığında (`pytest modules/store_reports/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_reports_backend" not in sys.modules:
    package = types.ModuleType("store_reports_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_reports_backend"] = package
