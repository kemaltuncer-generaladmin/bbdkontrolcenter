...
modülün `backend` klasörü `store_refunds_backend` adıyla kaydedilir;
içerideki göreli importlar (`.calc`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/store_refunds/tests`) yüklenmez.
# Paketleme yapıldığında bu satır da kök conftest ile birlikte kalkar.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_refunds_backend" not in sys.modules:
    package = types.ModuleType("store_refunds_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_refunds_backend"] = package
