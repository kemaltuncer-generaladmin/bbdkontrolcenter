...
`bld_api_backend` adıyla kaydedilir; içerideki göreli importlar (`.errors`)
...
if "bld_api_backend" not in sys.modules:
    package = types.ModuleType("bld_api_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["bld_api_backend"] = package
