...
`backend` klasörü `store_orders_backend` adıyla kaydedilir; içerideki göreli
importlar (`.orders`) paketin `__path__` değeri üzerinden çözülür.
"""
...
# çalıştırıldığında (`pytest modules/store_orders/tests`) yüklenmez.
# Paketleme yapıldığında bu satır da kök conftest ile birlikte kalkar.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_orders_backend" not in sys.modules:
    package = types.ModuleType("store_orders_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_orders_backend"] = package
