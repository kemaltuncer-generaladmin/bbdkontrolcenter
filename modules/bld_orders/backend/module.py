"""Sipariş Yönetimi — giriş noktası.
...
durumu anlatır (K7) — siparişler akmaya devam ediyorken yönetim ekranının
...
#: Raporlar/BLD/Siparişler/<yıl>/<ay>
CATEGORY = "BLD"
SUBCATEGORY = "Siparişler"
...
def register(ctx: ModuleContext) -> None:
    ...
    service = OrdersService(
        ...
        # Masaüstü bulunamazsa çıktı depo içindeki `data/exports`a düşer;
        # dışa aktarım üretilemeyip iş durmaz (`km_core.files.outputs`).
        fallback_dir=ctx.module_path.parents[1] / "data" / "exports",
    )
    ctx.add_router(bind(service))
    ctx.log.info("sipariş yönetimi hazır",
