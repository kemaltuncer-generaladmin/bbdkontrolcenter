...
`backend` klasörü `bld_orders_backend` adıyla kaydedilir; içerideki göreli
...
# çalıştırıldığında (`pytest modules/bld_orders/tests`) yüklenmez.
# Paketleme yapıldığında bu satır da kök conftest ile birlikte kalkar.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "bld_orders_backend" not in sys.modules:
    package = types.ModuleType("bld_orders_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["bld_orders_backend"] = package
