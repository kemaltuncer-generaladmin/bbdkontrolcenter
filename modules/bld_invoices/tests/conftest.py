...
`backend` klasörü `bld_invoices_backend` adıyla kaydedilir; içerideki göreli
importlar (`.documents`) paketin `__path__` değeri üzerinden çözülür.
"""
...
# çalıştırıldığında (`pytest modules/bld_invoices/tests`) yüklenmez.
# Paketleme yapıldığında bu satır da kök conftest ile birlikte kalkar.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "bld_invoices_backend" not in sys.modules:
    package = types.ModuleType("bld_invoices_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["bld_invoices_backend"] = package
