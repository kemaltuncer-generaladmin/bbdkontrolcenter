...
durumu anlatır (K7) — telefon çalmaya devam ediyorken sipariş ekranının hiç
...
`ctx.store` KULLANILMIYOR ve bu bilinçli: modülün göçü yoktur, dolayısıyla
yazabileceği bir tablosu da yoktur (K5). Uzak verinin kopyası tutulmaz; yazma
izini geçit `mod_bld_api_audit` içinde tutuyor.
"""
...
def register(ctx: ModuleContext) -> None:
    ...
    service = ManualOrderService(api=api, log=ctx.log, config=ctx.config)
    ctx.add_router(bind(service))
    ctx.log.info("elle sipariş ekranı hazır",
