...
`backend` klasörü `bld_kds_backend` adıyla kaydedilir; içerideki göreli
importlar (`.devices`) paketin `__path__` değeri üzerinden çözülür.
"""
...
# çalıştırıldığında (`pytest modules/bld_kds/tests`) yüklenmez.
# Paketleme yapıldığında bu satır da kök conftest ile birlikte kalkar.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "bld_kds_backend" not in sys.modules:
    package = types.ModuleType("bld_kds_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["bld_kds_backend"] = package
