...
`backend` klasörü `store_tax_backend` adıyla kaydedilir; içerideki göreli
importlar (`.taxes`, `.summary`) paketin `__path__` değeri üzerinden çözülür.
"""
...
# çalıştırıldığında (`pytest modules/store_tax/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_tax_backend" not in sys.modules:
    package = types.ModuleType("store_tax_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_tax_backend"] = package
