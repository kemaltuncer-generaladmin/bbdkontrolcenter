...
modülün `backend` klasörü `store_home_media_backend` adıyla kaydedilir;
içerideki göreli importlar (`.slots`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/store_home_media/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_home_media_backend" not in sys.modules:
    package = types.ModuleType("store_home_media_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_home_media_backend"] = package
