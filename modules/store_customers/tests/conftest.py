...
modülün `backend` klasörü `store_customers_backend` adıyla kaydedilir;
içerideki göreli importlar (`.analytics`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/store_customers/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_customers_backend" not in sys.modules:
    package = types.ModuleType("store_customers_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_customers_backend"] = package
