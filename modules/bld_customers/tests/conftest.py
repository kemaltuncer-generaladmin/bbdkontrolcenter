...
`backend` klasörü `bld_customers_backend` adıyla kaydedilir; içerideki göreli
importlar (`.people`) paketin `__path__` değeri üzerinden çözülür.
"""
...
# çalıştırıldığında (`pytest modules/bld_customers/tests`) yüklenmez.
# Paketleme yapıldığında bu satır da kök conftest ile birlikte kalkar.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "bld_customers_backend" not in sys.modules:
    package = types.ModuleType("bld_customers_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["bld_customers_backend"] = package
