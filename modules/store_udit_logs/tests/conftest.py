...
modülün `backend` klasörü `store_udit_logs_backend` adıyla kaydedilir;
içerideki göreli importlar (`.records`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/store_udit_logs/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_udit_logs_backend" not in sys.modules:
    package = types.ModuleType("store_udit_logs_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_udit_logs_backend"] = package
