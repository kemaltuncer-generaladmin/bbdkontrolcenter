...
modülün `backend` klasörü `store_shipping_backend` adıyla kaydedilir;
içerideki göreli importlar (`.shipping`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/store_shipping/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_shipping_backend" not in sys.modules:
    package = types.ModuleType("store_shipping_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_shipping_backend"] = package
