...
modülün `backend` klasörü `store_invoices_backend` adıyla kaydedilir;
içerideki göreli importlar (`.ledger`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/store_invoices/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_invoices_backend" not in sys.modules:
    package = types.ModuleType("store_invoices_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_invoices_backend"] = package
