...
`backend` klasörü `bld_sales_settings_backend` adıyla kaydedilir; içerideki
göreli importlar (`.settings`) paketin `__path__` değeri üzerinden çözülür.
"""
...
# çalıştırıldığında (`pytest modules/bld_sales_settings/tests`) yüklenmez.
# Paketleme yapıldığında bu satır da kök conftest ile birlikte kalkar.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "bld_sales_settings_backend" not in sys.modules:
    package = types.ModuleType("bld_sales_settings_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["bld_sales_settings_backend"] = package
