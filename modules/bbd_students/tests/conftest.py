...
modülün `backend` klasörü `bbd_students_backend` adıyla kaydedilir; içerideki
göreli importlar (`.code_list`, `.cards`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/bbd_students/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "bbd_students_backend" not in sys.modules:
    package = types.ModuleType("bbd_students_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["bbd_students_backend"] = package
