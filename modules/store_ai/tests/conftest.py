...
modülün `backend` klasörü `store_ai_backend` adıyla kaydedilir; içerideki
göreli importlar (`.rules`, `.analytics`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/store_ai/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_ai_backend" not in sys.modules:
    package = types.ModuleType("store_ai_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_ai_backend"] = package
