...
modülün `backend` klasörü `store_promotions_backend` adıyla kaydedilir;
içerideki göreli importlar (`.promotions`) paketin `__path__` değeri üzerinden
çözülür.
"""
...
# başına çalıştırıldığında (`pytest modules/store_promotions/tests`) yüklenmez.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "store_promotions_backend" not in sys.modules:
    package = types.ModuleType("store_promotions_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["store_promotions_backend"] = package
