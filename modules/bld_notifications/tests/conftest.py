...
`backend` klasörü `bld_notifications_backend` adıyla kaydedilir; içerideki
göreli importlar (`.notices`) paketin `__path__` değeri üzerinden çözülür.
"""
...
# çalıştırıldığında (`pytest modules/bld_notifications/tests`) yüklenmez.
# Paketleme yapıldığında bu satır da kök conftest ile birlikte kalkar.
SRC = ROOT / "backend" / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

if "bld_notifications_backend" not in sys.modules:
    package = types.ModuleType("bld_notifications_backend")
    package.__path__ = [str(BACKEND)]  # type: ignore[attr-defined]
    sys.modules["bld_notifications_backend"] = package
